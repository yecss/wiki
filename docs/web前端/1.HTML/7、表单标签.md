...
<form action="https://example.com/api" method="post">
    ...
    <!-- 表示向服务器发送一个键名为user的键值对，键值就是这个控件的value值 -->
...
- `for` ：关联控件的id属性，使得点击label就可以 聚焦到对应控件
- `form` 关联表单的id属性，关联后label 就可以写在任意的位置。否则只能放在form内部。
...
<label for="post-user">用户名：</label>
<input id="post-user" type="search" name="wd" required>
